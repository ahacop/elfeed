...
(defvar elfeed--entities-hash
      #s(hash-table
         ...
         data (
               ...
               "&Agrave;" "À" "&Aacute;" "Á" "&Acirc;" "Â" "&Atilde;"
               "Ã" "&Auml;" "Ä" "&Aring;" "Å" "&AElig;" "Æ" "&Ccedil;"
               "Ç" "&Egrave;" "È" "&Eacute;" "É" "&Ecirc;" "Ê" "&Euml;"
               "Ë" "&Igrave;" "Ì" "&Iacute;" "Í" "&Icirc;" "Î" "&Iuml;"
               "Ï" "&ETH;" "Ð" "&Ntilde;" "Ñ" "&Ograve;" "Ò" "&Oacute;"
               "Ó" "&Ocirc;" "Ô" "&Otilde;" "Õ" "&Ouml;" "Ö" "&times;"
               ...
               "&ccedil;" "ç" "&egrave;" "è" "&eacute;" "é" "&ecirc;"
               "ê" "&euml;" "ë" "&igrave;" "ì" "&iacute;" "í" "&icirc;"
               "î" "&iuml;" "ï" "&eth;" "ð" "&ntilde;" "ñ" "&ograve;"
               "ò" "&oacute;" "ó" "&ocirc;" "ô" "&otilde;" "õ" "&ouml;"
               "ö" "&divide;" "÷" "&oslash;" "ø" "&ugrave;" "ù"
               "&uacute;" "ú" "&ucirc;" "û" "&uuml;" "ü" "&yacute;" "ý"
               "&thorn;" "þ" "&yuml;" "ÿ" "&fnof;" "ƒ" "&Alpha;" "Α"
               "&Beta;" "Β" "&Gamma;" "Γ" "&Delta;" "Δ" "&Epsilon;" "Ε"
               "&Zeta;" "Ζ" "&Eta;" "Η" "&Theta;" "Θ" "&Iota;" "Ι"
               "&Kappa;" "Κ" "&Lambda;" "Λ" "&Mu;" "Μ" "&Nu;" "Ν" "&Xi;"
...
(defun elfeed--entity->character (entity)
  (gethash entity elfeed--entities-hash entity))

(defun elfeed--strip-html-tags (html)
  (replace-regexp-in-string "\\(<[^>]+>\\)" "" html))

(defun elfeed--decode-html-entities (html)
  (replace-regexp-in-string "\\(&[^;]+;\\)" 'elfeed--entity->character html))

(defun elfeed--decode-html (html)
  (elfeed--decode-html-entities (elfeed--strip-html-tags html)))
...
(defun elfeed-entries-from-atom (url xml)
  ...
  (let* ((feed-id url)
    ...
    (cl-loop for entry in (xml-query-all* (feed entry) xml) collect
             (let* ((title-type (or (xml-query* (title :type) entry) ""))
                    (title (or (xml-query* (title *) entry)
                               (xml-query* (title b *) entry)
                               ""))
                    ...
                    (db-entry (elfeed-entry--create
                               :title (if (or (string-match-p "html" title-type)
                                              (string-match-p "xhtml" title-type))
                                        (elfeed--decode-html (elfeed-cleanup title))
                                        (elfeed-cleanup title))
                               :feed-id feed-id
